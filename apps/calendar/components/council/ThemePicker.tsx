...
type ThemeOption = { value: string; label: string; swatch: string; accent: string };

// Kept in sync with the [data-theme="…"] blocks in globals.css. "parchment"
// is the default light look (the :root values), so it needs no override block.
...
/** Compact theme swatch list for the profile menu. Shared across zones via
 *  same-origin localStorage; the root-layout inline script reapplies the
      ...
      {THEME_OPTIONS.map((t) => {
        ...
        return (
          <button
            ...
            className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-left outline-none transition hover:bg-parchment"
          >
            <span
